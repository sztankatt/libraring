...
from usr.project import PROJECT_NAME, REGISTRATION_EMAIL, PROJECT_URL, user_is_not_blocked, ProfileUpdate,\
...
def deactivated_profile(request):
    ...
    return render(request, 'before_login/messages.html', {'user':user, 'message': PageMessages.objects.get(name='deactivated_profile')})
...
def register_confirmation(request):
	user = User.objects.get(pk=request.session['last_logged_user'])
	return render(request, 'before_login/messages.html', {'user': user, 'message':PageMessages.objects.get(name='registry_confirmation')})
...
def register_confirmation_code(request, confirmation_code=None):
    ...
    return render(request, 'before_login/messages.html', {'user': user, 'registration_complete':True, 'message':PageMessages.objects.get(name='registry_confirmation_correct')})
